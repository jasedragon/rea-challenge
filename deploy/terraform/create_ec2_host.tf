...
resource "null_resource" "run-provisioner" {
  provisioner "local-exec" {
    command = "ansible-playbook -u '${var.ansible_ssh_user}' --private-key $KEY --extra-vars='ec2_ip=${aws_instance.ec2machine.public_ip}' -i '${aws_instance.ec2machine.public_ip},' ../ansible/master.yml"
